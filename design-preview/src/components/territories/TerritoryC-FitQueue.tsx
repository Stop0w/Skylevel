...
const TerritoryCFitQueue: React.FC = () => {
  ...
  const getFitScoreColor = (score: number) => {
    if (score >= 85) return 'bg-accent-100 text-accent-800 border-accent-300'
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {sortedCandidates.map((candidate) => (
                <tr key={candidate.id} className="hover:bg-gray-50">
                  ...
                  <td className="w-16 px-2 py-2">
                    <div className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold border ${getFitScoreColor(candidate.fitScore)}`}>
                      {candidate.fitScore}
                    </div>
                  </td>
